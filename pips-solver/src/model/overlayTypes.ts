...
export interface AIExtractionResult {
  ...
  /** Debug information including per-model raw responses (when saveDebugResponses is enabled) */
  debug?: {
...
// ════════════════════════════════════════════════════════════════════════════
// Debug Response Types (for extraction comparison)
// ════════════════════════════════════════════════════════════════════════════

/**
 * Captures a single model's response including raw text, parsed result, and timing
 */
export interface ModelResponse<T = unknown> {
  /** Model identifier (e.g., 'gemini-2.0-flash-exp', 'claude-sonnet-4', 'gpt-4o') */
  model: string;
  /** Raw text response from the model before parsing */
  rawText: string;
  /** Parsed and validated data from the response */
  parsedData: T | null;
  ...
  timing: {
    /** Time to receive response from model */
    responseMs: number;
    /** Time to parse and validate response */
    parseMs: number;
  };
  /** Confidence score reported by this model (0-1) */
  confidence?: number;
}

/**
 * Board-specific model response with typed parsed data
 */
export type BoardModelResponse = ModelResponse<BoardExtractionResult>;

/**
 * Domino-specific model response with typed parsed data
 */
export type DominoModelResponse = ModelResponse<DominoExtractionResult>;

/**
 * Collection of all raw model responses from an extraction run
 * Used for comparison/diff view to show per-model results
 */
export interface RawResponses {
  /** Per-model responses for board extraction */
  board: BoardModelResponse[];
  /** Per-model responses for domino extraction */
  dominoes: DominoModelResponse[];
  /** Model selected as "best" for final result (board) */
  selectedBoardModel?: string;
  /** Model selected as "best" for final result (dominoes) */
  selectedDominoModel?: string;
  /** Overall timing for the extraction */
  timing: {
    boardMs: number;
    dominoesMs: number;
    totalMs: number;
